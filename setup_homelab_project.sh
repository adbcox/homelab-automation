...
echo "  curl -X POST http://127.0.0.1:8000/run/all-checks"
EOF
...
app = FastAPI(title="Homelab Automation API", version="1.0.0")
...
echo "Test commands:"
echo "  curl http://127.0.0.1:8000/health"
echo "  curl -X POST http://127.0.0.1:8000/run/all-checks"chmod +x setup_homelab_project.sh
./setup_homelab_project.sh
./bootstrap.sh
./start_api.sh#!/usr/bin/env bash
set -euo pipefail
...
echo "Then test:"
echo "  curl http://127.0.0.1:8000/health"
echo "  curl -X POST http://127.0.0.1:8000/run/all-checks -H 'X-API-Token: YOUR_TOKEN'"
""",
    encoding="utf-8",
)

print("Updated .env.example, api_server.py, and bootstrap.sh")
PY

echo
echo "Done."
echo "Next:"
echo "  1) Edit .env and set API_TOKEN"
echo "  2) ./bootstrap.sh"
